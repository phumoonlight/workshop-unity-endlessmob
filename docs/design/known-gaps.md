...
- The Assassin already plays the pack's twin-dagger (`WTD`) animations but still **holds a bow** (`BowProp` under `ModelBow`) while throwing daggers. The Haons pack has twin-dagger hand props (`Prefab/Prop_AttachToHand(OptUseAnim)`) that could replace it.
