...
## The hero
- **Body vs model:** the Player is a capsule with a `CharacterController` — all gameplay runs on it and its mesh is hidden. `ModelSword` / `ModelBow` are children (`ModelBow` is the Assassin's body — it still holds a bow, and code and fields still say "bow" / "arrows" from its Archer days); `PlayerModel` shows the chosen one and feeds `CharacterController.velocity` into a `Speed` blend tree (0 / 3 / 6 m/s). Root motion is off on purpose.
...
- **Finding enemies — the `Enemy` physics layer (slot 6).** Every "who is near?" query (`Weapon.FindNearestEnemy`, `MeleeSlash`, `WhirlwindSkill`, `BouncingBlade`, `BouncingBladeSkill`, `CampCapture`, and the shelved `Soldier` / `Builder`) passes `Layers.EnemyMask` to `Physics.OverlapSphereNonAlloc`, so physics returns enemies only instead of every collider in range. `Layers` (in `Core`) is the one place that knows layer names. The root of each enemy prefab (where the collider is) is on the layer; **a new enemy prefab must be too**, or no attack can find it — `Enemy.Awake` fixes a forgotten one at runtime and logs a warning. The layer collision matrix is untouched (everything still collides with everything).
...
- **`SettingsMenu`** (on the Game `UI` canvas): builds the gear button (top-right, 56 px, icon from `HudIcons.Gear`) and the settings window — one SOUND section with three `UiFactory.Slider`s. Opening sets `timeScale` 0 and remembers whether time was running (`> 0`, because hit-stop runs at 5 %), so closing never restarts a pause it did not start (level-up popup, Esc pause). It closes on Esc in **`LateUpdate`** and exposes static `IsOpen`; `DebugHUD` ignores Esc / M and skips its OnGUI text while it is open — otherwise one Esc press would close the window *and* toggle pause, and OnGUI would draw over it. Dragging a slider previews its sound at most every 0.25 s (unscaled time). The DEV button and panel were moved down 44 px to make room.
- **`SoundSettings`** (static) holds `Master` / `Slash` / `Shoot` (defaults 0.8 / 0.25 / 0.15), loaded from **PlayerPrefs** before the first scene and saved when the window closes. Settings are not progress, so they stay out of `profile.json`; editor and built game have separate PlayerPrefs. `GameAudio` reads them on every play.
- **`DebugHUD`** (OnGUI): FPS + live mob count bottom-right, and the Esc pause screen (M quits to the menu). After a run the buttons are on `WaveHud`'s result panel, not here.
...
## Presentation
- **Audio:** `GameAudio` creates itself and finds each sound by name in `Resources/Sfx/` — a file, or a **folder of takes** (one picked at random, never the same twice). `Sfx/Slash/` holds the sword swing: one take (mp3 source, imported as Vorbis mono, normalised) that replaced 28 older ones, which are still in git history. `Sfx/Shoot.ogg` (0.45 s, mono) is the Assassin's dagger throw, played by `AutoShooter` and `BouncingBladeSkill` — a single take for now; turn it into a `Shoot/` folder of takes if it gets repetitive under Rapid Fire. A missing sound is remembered as missing until the next Play press. Loudness = the caller's volume × `BaseVolume(sfx)` × `SoundSettings.Master`: the two constant sounds (`Slash`, `Shoot`) have their own volume in `SoundSettings`, everything else is 1. Every other sound is silent: the generated placeholders are off (`playGeneratedSounds`). The swing is loudest at 0.20 s (quiet before 0.16 s), so `MeleeSlash` starts the sound at the wind-up.
