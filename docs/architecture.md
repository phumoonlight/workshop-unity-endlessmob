...
- **Skills:** `SkillController` drives `ClassSkill` subclasses (`WhirlwindSkill`, `BarrageSkill`, `RapidFireSkill`).
...
- **Ids, not names.** Items save by `ItemData.id` (Berry 1, Relic 2, Armor 3 — next free 4) and classes by `PlayerClassData.id` (Swordsman 1, Archer 2 — next free 3), so assets can be renamed freely. **Never change or reuse an id.** Duplicate or unset (0) ids log errors.
